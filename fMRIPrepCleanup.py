...
if 'sim' in method:
	...
	#Create a new folder for symbolic links. If the folder already exists, it's deleted. The exact path depends on whether copy or
	...
	for root, dirs, files in os.walk(fmriprep_dir, topdown = False):
		for filename in files:
			...
			#However, we do look through these folders if the to_keep command has been provided.
			if 'single_subject' in file_path or 'fsaverage' in file_path:
				if '-also_keep' in arg_dict:
					if not any(keep_string in filename for keep_string in also_keep):
						continue
				else:
					continue

			#For any files that contain one of our target strings (and aren't specified in also_delete), a symbolic link
			#or copy will be made of them using their relative path within the fMRIPrep directory.
				...
				sim_path = os.path.join(sim_dir, os.path.relpath(file_path, fmriprep_dir))
